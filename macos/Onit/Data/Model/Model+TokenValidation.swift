...
struct TokenValidationState {
    private var states: [ModelProvider: ValidationState] = [:]
    ...
    mutating func setValidating(provider: ModelProvider) {
        states[provider] = .validating
    }
    
    mutating func setValid(provider: ModelProvider) {
        states[provider] = .valid
    }
    
    mutating func setInvalid(provider: ModelProvider, error: Error) {
        states[provider] = .invalid(error)
    }
    
    func state(for provider: ModelProvider) -> ValidationState {
...
extension OnitModel {
    ...
    @MainActor
    func validateToken(provider: ModelProvider, token: String) async {
