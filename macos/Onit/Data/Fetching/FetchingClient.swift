...
actor FetchingClient {
    ...
    func chat(
        instructions: [String], inputs: [Input?], files: [[URL]], images: [[URL]],
        autoContexts: [[String: String]], responses: [String], model: AIModel?, apiToken: String?
    ) async throws -> String {
        ...
        case .openAI:
                    ...
                    for url in images[index] {
                        if let imageData = try? Data(contentsOf: url) {
                            ...
                            let imagePart = OpenAIChatContentPart(
                                ...
                                text: nil,
                                image_url: .init(
                                    url: "data:\(mimeType);base64,\(base64EncodedData)")
                            )
            ...
            let endpoint = OpenAIChatEndpoint(
                messages: openAIMessageStack, token: apiToken, model: model.id)
            ...
            for (index, userMessage) in userMessages.enumerated() {
                ...
                if index < responses.count {
                    let assistantContent = [
                        AnthropicContent(type: "text", text: responses[index], source: nil)
                    ]
        ...
        case .xAI:
            ...
            let endpoint = XAIChatEndpoint(
                messages: xAIMessageStack, model: model.id, token: apiToken)
            let response = try await execute(endpoint)
            return response.choices[0].message.content
        ...
        case .googleAI:
            ...
            if model.supportsSystemPrompts {
                googleAIMessageStack.append(
                    GoogleAIChatMessage(role: "system", content: .text(systemMessage)))
            }

            for (index, userMessage) in userMessages.enumerated() {
                if images[index].isEmpty {
                    googleAIMessageStack.append(
                        GoogleAIChatMessage(role: "user", content: .text(userMessage)))
                } else {
                    let parts =
                        [
                    ...
                    googleAIMessageStack.append(
                        GoogleAIChatMessage(role: "user", content: .multiContent(parts)))
                }
            ...
            let endpoint = GoogleAIChatEndpoint(
                messages: googleAIMessageStack, model: model.id, token: apiToken)
            let response = try await execute(endpoint)
            return response.choices[0].message.content
        ...
        case .deepSeek:
            ...
            if model.supportsSystemPrompts {
                deepSeekMessageStack.append(
                    DeepSeekChatMessage(role: "system", content: .text(systemMessage)))
            }

            for (index, userMessage) in userMessages.enumerated() {
                if images[index].isEmpty {
                    deepSeekMessageStack.append(
                        DeepSeekChatMessage(role: "user", content: .text(userMessage)))
                } else {
                    var parts = [
                        DeepSeekChatContentPart(type: "text", text: userMessage, image_url: nil)
                    ]
                    for url in images[index] {
                        if let imageData = try? Data(contentsOf: url) {
                            let base64EncodedData = imageData.base64EncodedString()
                    ...
                    deepSeekMessageStack.append(
                        DeepSeekChatMessage(role: "user", content: .multiContent(parts)))
                }
            ...
            let endpoint = DeepSeekChatEndpoint(
                messages: deepSeekMessageStack, token: apiToken!, model: model.id)
            let response = try await execute(endpoint)
            return response.choices[0].message.content
        ...
        case .custom:
            ...
            if let customProvider = Defaults[.availableCustomProviders].first(where: {
                $0.name == model.customProviderName
            }) {
                let url = URL(string: customProvider.baseURL)!
                let endpoint = CustomChatEndpoint(
                    baseURL: url, messages: openAIMessageStack, token: customProvider.token,
                    model: model.id)
                let response = try await execute(endpoint)
                return response.choices[0].message.content
            } else {
