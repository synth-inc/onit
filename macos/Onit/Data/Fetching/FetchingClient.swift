...
actor FetchingClient {
    ...
    func chat(instructions: [String], inputs: [Input?], files: [[URL]], images: [[URL]], autoContexts: [[String: String]], responses: [String], model: AIModel?, apiToken: String?) async throws -> String {
        ...
        case .deepSeek:
            var deepSeekMessageStack: [OpenAIChatMessage] = []
            
            // DeepSeek uses OpenAI-compatible format
            if model.supportsSystemPrompts {
                deepSeekMessageStack.append(OpenAIChatMessage(role: "system", content: .text(systemMessage)))
            }
            
            for (index, userMessage) in userMessages.enumerated() {
                if images[index].isEmpty {
                    deepSeekMessageStack.append(OpenAIChatMessage(role: "user", content: .text(userMessage)))
                } else {
                    var parts = [OpenAIChatContentPart(type: "text", text: userMessage, image_url: nil)]
                    for url in images[index] {
                        if let imageData = try? Data(contentsOf: url) {
                    ...
                    deepSeekMessageStack.append(OpenAIChatMessage(role: "user", content: .multiContent(parts)))
                }
            ...
            let endpoint = DeepSeekChatEndpoint(messages: deepSeekMessageStack, model: model.id, token: apiToken!)
            let response = try await execute(endpoint)
            return response.choices[0].message.content
