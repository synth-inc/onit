...
class HighlightHintWindowController {
  ...
  func insertText(_ text: String) {
    ...
    let result = AXUIElementCopyAttributeValue(
      systemElement, kAXFocusedUIElementAttribute as CFString, &focusedElementRef)
    ...
    let isSettableResult = AXUIElementIsAttributeSettable(
    ...
    if isSettableResult == .success && isSettable.boolValue {
      ...
      if valueResult == .success, let currentValue = valueRef as? String {
        ...
        if rangeResult == .success {
          ...
          if AXValueGetValue(selectedRangeValue, .cfRange, &range) {
            ...
            let newText = nsCurrentValue.replacingCharacters(
      ...
      print(
        "Element's value attribute is not settable or failed to check. Error: \(isSettableResult.rawValue)"
      )
    }
  }
}
