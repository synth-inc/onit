...
extension FileRow {
        ...
        if accessibilityEnabled {
               ...
               !windowState.pinnedPendingInputs.contains(trackedPendingInput)
            {
                ghostContextTag(
                    ...
                    addTrackedHighlightedTextToContext(trackedPendingInput)
                }
    ...
    private var unpinnedHighlightedTextContextItem: some View {
        ...
        {
            ContextTag(
                text: unpinnedPendingInput.selectedText,
                iconView: Image(.text).addIconStyles(iconSize: 14)
            ) {
...
extension FileRow {
    ...
    private func addTrackedHighlightedTextToContext(_ trackedPendingInput: Input) {
        guard let windowState = windowState else { return }
