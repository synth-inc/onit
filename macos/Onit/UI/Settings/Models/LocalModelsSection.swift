...
struct LocalModelsSection: View {
    ...
    private var localOptionsBinding: Binding<LocalChatOptions> {
        Binding {
            LocalChatOptions(num_ctx: localNumCtx,
                             temperature: localTemperature,
                             top_p: localTopP,
                             top_k: localTopK)
        } set: { newValue in
            localNumCtx = newValue.num_ctx
            localTemperature = newValue.temperature
            localTopP = newValue.top_p
            localTopK = newValue.top_k
        }
    }
    ...
    var title: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            LocalModelAdvancedOptionsView(storedStreamResponse: $streamResponse.local,
                                          storedKeepAlive: $localKeepAlive,
                                          storedRequestTimeout: $localRequestTimeout,
                                          storedOptions: localOptionsBinding)
        }
    }
