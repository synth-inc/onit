...
extension PanelStatePinnedManager: AccessibilityNotificationsDelegate {
    func accessibilityManager(_ manager: AccessibilityNotificationsManager, didActivateWindow window: TrackedWindow) {
        guard let state = getState(for: window.element), state.panelOpened else { return }
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) { [weak self] in
            self?.resizeWindow(for: state, window: window.element)
        }
    }
    ...
    func accessibilityManager(_ manager: AccessibilityNotificationsManager, didMoveWindow window: TrackedWindow) {
        ...
        if !isDragging {
            resizeWindow(for: state, window: window.element)
        }
    }
    func accessibilityManager(_ manager: AccessibilityNotificationsManager, didResizeWindow window: TrackedWindow) {
        ...
        resizeWindow(for: state, window: window.element, windowFrameChanged: true)
    }
}
