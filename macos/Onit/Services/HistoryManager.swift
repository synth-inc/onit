...
final class HistoryManager: ObservableObject {
    ...
    private let queue = DispatchQueue(label: "com.onit.historyManager", qos: .background)

    let pasteboard: PasteboardHistoryManager
    let highlightedText: HighlightedTextHistoryManager
    let content: ContentHistoryManager
    let typedInput: TypedInputHistoryManager
    ...
    private init() {
        ...
        pasteboard = PasteboardHistoryManager(manager: self)
        highlightedText = HighlightedTextHistoryManager(manager: self)
        content = ContentHistoryManager(manager: self)
        typedInput = TypedInputHistoryManager(manager: self)
    }
...
final class PasteboardHistoryManager {
    private unowned let manager: HistoryManager
    init(manager: HistoryManager) { self.manager = manager }

    func add(text: String, applicationName: String, applicationTitle: String, timestamp: Date = Date()) {
        guard Defaults[.storeHistory] else { return }
            ...
            for row in rows {
                if let id = row["id"] as? Int64,
                   ...
                   let ts = row["timestamp"] as? Double {
                    result.append(PasteboardEntry(id: id, text: text, timestamp: Date(timeIntervalSince1970: ts)))
                }
            }
        }
        return result
            ...
            for row in rows {
                if let id = row["id"] as? Int64,
                   ...
                   let ts = row["timestamp"] as? Double {
                    ...
                    result.append(HighlightedTextEntry(id: id, text: text, app: app, window: window, timestamp: Date(timeIntervalSince1970: ts)))
                }
            }
        }
        return result
        ...
        manager.queue.sync {
            ...
            for row in rows {
                if let id = row["id"] as? Int64,
                   ...
                   let ts = row["timestamp"] as? Double {
                    let window = row["window"] as? String
                    result.append(ContentEntry(id: id, content: content, app: app, window: window, timestamp: Date(timeIntervalSince1970: ts)))
                }
            }
        }
        return result
