...
extension String {
    ...
    func levenshteinDistance(to other: String) -> Int {
        ...
        let (shorter, longer) = m < n ? (selfArray, otherArray) : (otherArray, selfArray)
        let (sLen, lLen) = (shorter.count, longer.count)

        var prev = Array(0...sLen)   // previous row
        var curr = Array(repeating: 0, count: sLen + 1) // current row
