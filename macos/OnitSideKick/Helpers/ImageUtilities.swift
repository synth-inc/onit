...
struct ImageUtilities {
    ...
    /// Saves a CGImage to disk as a PNG file in the temporary directory
    /// This operation runs on a background thread to avoid blocking the main UI thread
    /// - Parameters:
    ///   - cgImage: The CGImage to save
    ///   - prefix: A prefix to add to the filename for identification
    /// - Returns: The file path if successful, nil if failed
    static func saveCGImageToDisk(_ cgImage: CGImage?, folderName: String, prefix: String) async -> String? {
        ...
        return await Task.detached(priority: .utility) {
            if let documentsURL = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first {
                let imagesFolder = documentsURL.appendingPathComponent(folderName)
                ...
                do {
                    ...
                    print("Failed to save CGImage to disk: \(error)")
                    return nil
                }
            }
            return nil
