...
extension Context: Equatable, Hashable {
    static func == (lhs: Context, rhs: Context) -> Bool {
        ...
        case (.auto(let autoContext1), .auto(let autoContext2)):
            // Consider AutoContext equal based on stable identity only, so UI/windows persist across content updates
            return autoContext1.appHash == autoContext2.appHash &&
    ...
    func hash(into hasher: inout Hasher) {
        ...
        case .file(let url), .image(let url), .tooBig(let url), .error(let url, _), .web(let url, _, _):
            hasher.combine(url)
        case .auto(let autoContext):
            // Hash by stable identity only, to match equality semantics above
            hasher.combine(autoContext.appHash)
            hasher.combine(autoContext.appTitle)
