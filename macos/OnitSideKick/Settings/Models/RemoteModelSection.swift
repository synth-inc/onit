...
struct RemoteModelSection: View {
    ...
    @Default(.cerebrasToken) var cerebrasToken
    ...
    @Default(.useCerebras) var useCerebras
    ...
    @Default(.isCerebrasTokenValidated) var isCerebrasTokenValidated
    ...
    var streamResponseBinding: Binding<Bool> {
        ...
        case .cerebras:
            return $streamResponse.cerebras
        case .custom:
    ...
    var verifyButtonText: String {
        if validated {
            String.localized("Verified", table: "Models")
        } else {
            switch localState {
            case .validating:
                String.localized("Verifying", table: "Models")
            default:
                String.localized("Verify →", table: "Models")
            }
        }
    }
    ...
    var textField: some View {
        HStack(alignment: .center, spacing: 8) {
            SecureField(String(format: String.localized("Enter your %@ API key", table: "Models"), provider.title), text: $key)
            ...
            if validated {
                SimpleButton(
                    text: String.localized("Remove", table: "Models"),
                    action: {
                        tokenManager.tokenValidation.setNotValidated(provider: provider)
    ...
    var link: String {
        let linkText = String(format: String.localized("your %@ key", table: "Models"), provider.title)
        return "[\(linkText)](\(provider.url))"
    }

    var caption: some View {
        (Text(String.localized("You can put in ", table: "Models"))
            + Text(.init(link))
            + Text(String(format: String.localized(" to use %@ models at cost.", table: "Models"), provider.title)))
    ...
    var apiKeySettings: some View {
        DisclosureGroup(provider.title + " " + String.localized("API Key", table: "Models") + (validated ? " ✅" : ""), isExpanded: $showApiKey) {
    ...
    func save(key: String) {
        ...
        case .cerebras:
            cerebrasToken = key.isEmpty ? nil : key
        case .custom:
            break
        }
    }
    ...
    func checkValidated() {
        ...
        case .cerebras:
            validated = isCerebrasTokenValidated
        case .custom:
            break
        }
    }
    ...
    func save(validated: Bool) {
        ...
        case .cerebras:
            isCerebrasTokenValidated = validated
        case .custom:
            break
        }
    }
