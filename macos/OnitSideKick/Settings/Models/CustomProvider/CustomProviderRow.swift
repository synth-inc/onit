...
struct CustomProviderRow: View {
        ...
        VStack(alignment: .leading) {
            HStack {
                ...
                .alert(isPresented: $showAlert) {
                    Alert(
                        title: Text(String.localized("Remove Provider", table: "Models")),
                        message: Text(String.localized("Are you sure you want to remove this provider?", table: "Models")),
                        primaryButton: .destructive(Text(String.localized("Remove", table: "Models"))) {
                            removeProvider()
                        },
            ...
            if provider.isEnabled {
                GroupBox {
                    VStack {
                        TextField(String.localized("Search models", table: "Models"), text: $searchText)
    ...
    var tokenField: some View {
        HStack(spacing: 7) {
            TextField(String(format: String.localized("Enter your %@ API key", table: "Models"), provider.name), text: $provider.token)
            ...
            Button {
                ...
                Task {
                    ...
                    DispatchQueue.main.async {
                        if provider.isTokenValidated {
                            message = String.localized("Successfully verified token!", table: "Models")
                            isErrorMessage = false
                        } else {
                            message = String.localized("Failed to verify token.", table: "Models")
                            isErrorMessage = true
                        }
                        loading = false
                    }
                ...
                if provider.isTokenValidated {
                    Text(String.localized("Verified", table: "Models"))
                } else {
                    if loading {
                        ProgressView()
                            .controlSize(.small)
                    } else {
                        Text(String.localized("Verify →", table: "Models"))
                    }
                }
            }
            .disabled(loading)
