...
struct SettingsSystemPromptTable: View {
    ...
    var body: some View {
        Table(filteredPrompts, selection: $selected) {
            TableColumn(String.localized("Name", table: "Settings"), value: \.name)
            TableColumn(String.localized("Hotkey", table: "Settings")) { prompt in
                if let shortcut = KeyboardShortcuts.Name(prompt.id).shortcut?.native {
                    ...
                    Text(String.localized("record", table: "Settings")).foregroundColor(Color.gray)
                }
            }
            TableColumn(String.localized("Applications", table: "Settings")) { prompt in
                switch prompt.applications.count {
                case 0:
                    Text(String.localized("Add default", table: "Settings")).foregroundColor(Color.gray)
                case 1:
            ...
            TableColumn(String.localized("Tags", table: "Settings")) { prompt in
