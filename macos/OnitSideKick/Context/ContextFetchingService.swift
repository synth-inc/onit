...
class ContextFetchingService {
    ...
    func retrieveWindowContent(
        ...
        wasTriggeredAutomatically: Bool = false
    ) -> Task<Void, Never>? {
        ...
        let task = Task {
            if let documentInfo = findDocument(in: mainWindow) {
                handleWindowContent(
                    ...
                    wasTriggeredAutomatically: wasTriggeredAutomatically
                )
                ...
                if GoogleDriveService.shared.checkAuthorizationStatus() {
                    do {
                        ...
                        state.addAutoContext(trackedWindow: trackedWindow, screenResult: screenResult, wasTriggeredAutomatically: wasTriggeredAutomatically)
                    }
                } else {
    ...
    private func parseAccessibility(
        ...
        let results = await AccessibilityParsingManager.shared.parseElement(mainWindow, ignoreCache: true)
        handleWindowContent(
            results,
            for: state,
            trackedWindow: trackedWindow,
    ...
    private func handleWindowContent(
        ...
        if appBundleUrl == nil,
           let pid = trackedWindow?.pid ?? lastActiveWindowPid,
