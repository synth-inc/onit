...
struct HistoryView: View {
    ...
    var groupedChats: [String: [Chat]] {
        Dictionary(grouping: filteredChats) { chat in
            ...
            if calendar.isDateInToday(chat.timestamp) {
                return String.localized("Today", table: "Sidekick")
            } else if calendar.isDate(chat.timestamp, equalTo: now, toGranularity: .weekOfYear) {
                return String.localized("This Week", table: "Sidekick")
            } else if calendar.isDate(chat.timestamp, equalTo: now, toGranularity: .month) {
                return String.localized("This Month", table: "Sidekick")
            } else {
                return String.localized("Earlier", table: "Sidekick")
            }
        }
    }
    ...
    var body: some View {
        MenuList(
            header: MenuHeader(title: String.localized("History", table: "Sidekick")) {
                IconButton(
                    icon: .cross,
                    iconSize: 10
                ) {
...
extension HistoryView {
    private var emptyText: some View {
        Text(String.localized("No prompts found", table: "Sidekick"))
