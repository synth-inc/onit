...
struct TextViewWrapper: NSViewRepresentable {
    ...
    var textColor: NSColor = NSColor.S_0
    var placeholderColor: NSColor = NSColor.placeholderText
    ...
    func updateNSView(_ nsView: NSScrollView, context: Self.Context) {
        ...
        textView.updateRecordingIndicatorVisibility()
...
private class CustomTextView: NSTextView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        insertionPointColor = (audioRecorder.isRecording || audioRecorder.isTranscribing) ? NSColor.clear : textColor
    ...
    override func layout() {
        super.layout()
        updateRecordingIndicatorFrame()
    }

    /// Add or remove the recording indicator subview based on recording state.
    /// Called from updateNSView when state changes — never from draw().
    func updateRecordingIndicatorVisibility() {
        if audioRecorder.isRecording || audioRecorder.isTranscribing {
            ...
            needsLayout = true
        } else if recordingIndicator != nil {
            recordingIndicator?.removeFromSuperview()
            recordingIndicator = nil
        }
    }

    /// Update the recording indicator frame during layout.
    private func updateRecordingIndicatorFrame() {
        guard let recordingIndicator = recordingIndicator,
