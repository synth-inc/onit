...
struct ToolCallView: View {
    ...
    private var isMemoryTool: Bool {
        response.toolCallName?.contains("memory") == true
    }
    ...
    // MARK: - Memory Tool Header (custom style)

    private var memoryToolHeader: some View {
        HStack(spacing: 8) {
            // Brain icon
            Image(systemName: "brain")
                ...
                .padding(6)
                .background(Memory.iconBackgroundColor)
                .cornerRadius(6)

            // Title and content
            VStack(alignment: .leading, spacing: 2) {
                Text(String.localized("Memory Saved", table: "Sidekick"))
                    .foregroundColor(Color.T_1)
                    .font(.system(size: 13, weight: .medium))

                if let content = fullMemoryContent {
                    Text(content)
        ...
        .padding(.vertical, 10)
        .background(Memory.iconBackgroundColor.opacity(0.1))
        .cornerRadius(8)
        .overlay(
            RoundedRectangle(cornerRadius: 8)
                .stroke(Memory.iconBackgroundColor.opacity(0.2), lineWidth: 1)
        )
    }

    private var memoryContent: String? {
        guard let arguments = response.toolCallArguments,
    ...
    private var fullMemoryContent: String? {
        guard let arguments = response.toolCallArguments,
              let data = arguments.data(using: .utf8),
              let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let content = json["content"] as? String else {
        ...
        VStack(alignment: .leading, spacing: 16) {
                ...
                !arguments.isEmpty
            {
                VStack(alignment: .leading, spacing: 8) {
                    HStack {
                        Text(String.localized("Input", table: "Sidekick"))
                ...
                !result.isEmpty
            {
                VStack(alignment: .leading, spacing: 8) {
                    HStack {
                        Text(String.localized("Output", table: "Sidekick"))
                            .font(.system(size: 13, weight: .semibold))
                            .foregroundColor(Color.secondary)
    ...
    private var displayToolName: String {
        guard let toolName = response.toolCallName else {
            return String.localized("Tool Call", table: "Sidekick")
        }
