...
extension OnitPanelState {
    ...
    func createAndSavePrompt(accountId: Int?) {
        // Refresh window context elements before creating the prompt
        Task {
            await refreshWindowContextElements()
    ...
    func generate(_ prompt: Prompt) {
        ...
        let currentBundleId = (trackedWindow ?? foregroundWindow)?.pid.bundleIdentifier
        ...
        generateTask = Task { [systemPrompt, currentBundleId, weak self] in
            guard let self = self else { return }

            // Build memory selection context
            let selectionContext = MemorySelectionContext(
                appBundleIdentifier: currentBundleId,
                userInstruction: prompt.instruction,
                selectedText: prompt.input?.selectedText,
                autoContextText: self.extractAutoContextText(from: prompt.contextList)
            )

            // Fetch and score memories (only if enabled)
            var scoredMemories: [ScoredMemory] = []
            if Defaults[.memoriesEnabled] {
            ...
            // Enrich system prompt with memories
            let memoriesText = self.formatMemoriesForInjection(scoredMemories)
            let enrichedSystemMessage = systemPrompt.prompt + memoriesText
    ...
    func shouldUseStream(_ aiModel: AIModel) -> Bool {
        ...
        case .cerebras:
            return Defaults[.streamResponse].cerebras
        case .custom:
    ...
    private func executeToolCallSync(
        ...
        // Build tool context with current app info
        let currentBundleId = (trackedWindow ?? foregroundWindow)?.pid.bundleIdentifier
        let toolContext = ToolContext(appBundleIdentifier: currentBundleId)
    ...
    /// Formats scored memories for injection into the system prompt
    private func formatMemoriesForInjection(_ scoredMemories: [ScoredMemory]) -> String {
        guard !scoredMemories.isEmpty else { return "" }

        // Separate by relevance (score >= 3.0 means app-specific match)
        let highRelevance = scoredMemories.filter { $0.score >= 3.0 }
        let normalRelevance = scoredMemories.filter { $0.score < 3.0 }

        var result = "\n\n<memories>\n"

        if !highRelevance.isEmpty {
            result += "Key preferences for this context:\n"
            for item in highRelevance {
                result += "- \(item.memory.content)\n"
            }
        }

        if !normalRelevance.isEmpty {
            if !highRelevance.isEmpty {
                result += "\n"
            }
            result += "General preferences:\n"
            for item in normalRelevance {
                result += "- \(item.memory.content)\n"
            }
        }
    ...
    /// Extracts auto-context text from the context list
    private func extractAutoContextText(from contextList: [Context]) -> String? {
        let autoContextTexts = contextList.autoContexts.values.joined(separator: "\n")
        return autoContextTexts.isEmpty ? nil : autoContextTexts
    }
}
