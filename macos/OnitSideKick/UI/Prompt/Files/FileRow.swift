...
struct FileRow: View {
    ...
    private var windowBeingAddedToContext: Bool {
        ...
        if let foregroundWindow = windowState.foregroundWindow, !windowAlreadyInContext {
...
extension FileRow {
    ...
    private var addHighlightedTextToContextButton: some View {
           ...
           let trackedPendingInput = windowState.trackedPendingInput
        {
            ghostContextTag(
                ...
                tooltip: String.localized("Add Highlighted Text To Context", table: "Sidekick")
            ) {
    ...
    private var addWindowToContextButton: some View {
           ...
           let foregroundWindow = windowState.foregroundWindow
        {
            ...
            ghostContextTag(
                ...
                iconBundleURL: iconBundleURL,
                tooltip: String.localized("Add %@ Context", table: "Sidekick", foregroundWindowName)
            ) {
        ...
        Group {
            if let windowState = windowState {
                ForEach(Array(windowState.windowContextTasks.keys), id: \.self) { uniqueWindowIdentifier in
                    ...
                    // Skip showing loading UI if this window's context is marked as stale
                    // or if the window is already in context
                    if let trackedWindow = trackedWindow,
                       !isWindowAlreadyInContext(hash: uniqueWindowIdentifier) {
                        ContextTag(
    ...
    private func isWindowAlreadyInContext(hash: UInt) -> Bool {
        return contextList.contains { context in
            if case .auto(let autoContext) = context {
                return autoContext.appHash == hash
            }
            return false
        }
    }
    ...
    private var ocrDetailsLink: some View {
        ...
        if showOCRDetailsLink {
            HStack {
                Button(String.localized("Details →", table: "Sidekick")) {
                    showOCRDetails = true
                }
                .buttonStyle(.plain)
                .font(.system(size: 11))
