    ...
    var body: some View {
        MenuList(
            ...
            search: MenuList.Search(
                query: $searchQuery,
                placeholder: String.localized("Search windows, tabs & files", table: "Sidekick")
            )
        ) {
            ContextMenuWindows(
...
extension ContextMenu {
    ...
    private var menuHeader: MenuHeader<IconButton> {
        MenuHeader(title: showBrowserTabs ? String.localized("Browser tabs", table: "Sidekick") : String.localized("Add context", table: "Sidekick")) {
