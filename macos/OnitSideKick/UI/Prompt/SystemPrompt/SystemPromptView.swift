...
struct SystemPromptView: View {
    ...
    var body: some View {
        ...
        .onChange(of: windowState?.systemPromptId, initial: true) { _, systemPromptId in
            guard let prompts = try? modelContext.fetch(FetchDescriptor<SystemPrompt>()),
                ...
                errorMessage = String.localized("Could not find the selected prompt. Using default prompt instead.", table: "Sidekick")
                showErrorAlert = true
                return
            }
        ...
        .padding(.top, 8)
        .saveSize(in: $size)
        .alert(String.localized("Error", table: "Sidekick"), isPresented: $showErrorAlert) {
            Button(String.localized("OK", table: "Sidekick")) { }
        } message: {
            Text(errorMessage)
        }
    }
    ...
    private func addPrompt() {
        ...
        do {
            ...
            errorMessage = String.localized("Unable to save the prompt: %@", table: "Sidekick", error.localizedDescription)
            showErrorAlert = true
        }
    }
}
