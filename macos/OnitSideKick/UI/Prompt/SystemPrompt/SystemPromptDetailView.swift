    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            VStack(alignment: .leading, spacing: 8) {
                Text(String.localized("Prompt", table: "Sidekick"))
            ...
            if let shortcut = KeyboardShortcuts.Name(storedPrompt.id).shortcut?.native {
                VStack(alignment: .leading, spacing: 8) {
                    Text(String.localized("Hotkey", table: "Sidekick"))
                        .foregroundStyle(Color.S_1)
                        .bold()
            ...
            if !storedPrompt.applications.isEmpty || !storedPrompt.tags.isEmpty {
                VStack(alignment: .leading, spacing: 8) {
                    Text(String.localized("Apps & Tags", table: "Sidekick"))
                        .foregroundStyle(Color.S_1)
                        .bold()
                    appsAndTags
                }
    ...
    private var buttons: some View {
        HStack(alignment: .center) {
            Button {
                ...
                Text(String.localized("Select another prompt", table: "Sidekick"))
                    .foregroundStyle(Color.S_1)
                    .underline()
            }.buttonStyle(.plain)
            ...
            if let windowState = windowState, windowState.systemPromptId != SystemPrompt.outputOnly.id {
                Button {
                    ...
                    Image(.pencil)
                        ...
                        .frame(width: 16, height: 16)
                        .foregroundStyle(Color.S_1)
                }.tooltip(prompt: String.localized("Edit", table: "Sidekick"))
            }
        }
    }
}
