        ...
        if let currentChat = windowState.currentChat {
            if !currentChat.isEmpty {
                ...
                if let keyboardShortcutString = KeyboardShortcuts.getShortcut(for: .newChat)?
                    .description
                {
                    return String.localized("Follow-up... (%@ for new), / for actions", table: "Sidekick", keyboardShortcutString)
                } else {
                    return String.localized("Follow-up..., / for actions", table: "Sidekick")
                }

            } else {
                return String.localized("New instructions, / for actions", table: "Sidekick")
            }
        } else {
            return String.localized("New instructions, / for actions", table: "Sidekick")
        }
