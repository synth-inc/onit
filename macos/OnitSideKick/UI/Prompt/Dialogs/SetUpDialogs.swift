...
//            #endif
    ...
    var noRemote: some View {
        SetUpDialog(
            title: String.localized("Couldn't Get Remote Models", table: "Sidekick"), buttonText: fetchingRemote ? String.localized("Loading...", table: "Sidekick") : String.localized("Retry", table: "Sidekick")
        ) {
            Text(String.localized("Onit couldn't load remote models - check your internet connection and try again!", table: "Sidekick"))
        } action: {
            Task {
                fetchingRemote = true
    ...
    func newRemote(models: [AIModel]) -> some View {
        SetUpDialog(title: String.localized("NEW Models Available!", table: "Sidekick"), buttonText: String.localized("Enable in Settings", table: "Sidekick")) {
            ...
            Text(String.localized("New models from %@. View and enable them in settings.", table: "Sidekick", newModelsText))
        } action: {
            settings()
    ...
    func deprecatedRemote(models: [AIModel]) -> some View {
        SetUpDialog(title: String.localized("Deprecated Models", table: "Sidekick"), buttonText: String.localized("Disable in Settings", table: "Sidekick")) {
            ...
            Text(
                String.localized("The following models are deprecated %@. You can disable them in settings.", table: "Sidekick", deprecatedModelsText)
            )
        } action: {
            settings()
    ...
    var local: some View {
        SetUpDialog(title: String.localized("Set Up Local Models", table: "Sidekick")) {
            Text(String.localized("Get ", table: "Sidekick"))
                + Text("[Ollama](https://ollama.com/download/mac)")
                + Text(
                    String.localized(" to connect to local models that run directly on your computer for added privacy.", table: "Sidekick")
                )
        } action: {
            settings()
        } closeAction: {
    ...
    var restartLocal: some View {
        SetUpDialog(
            title: String.localized("No Local Models Found", table: "Sidekick"), buttonText: fetchingLocal ? String.localized("Loading...", table: "Sidekick") : String.localized("Try again", table: "Sidekick")
        ) {
            Text(String.localized("Onit couldn't connect to local models - you may need to restart Ollama.", table: "Sidekick"))
        } action: {
            Task {
                fetchingLocal = true
