...
struct MicrophoneButton: View {
    ...
    var body: some View {
        PromptCoreFooterButton(
            iconColor: audioRecorder.isRecording ? Color.red500 : Color.S_2,
            icon: .microphone,
            text: audioRecorder.isRecording ? String.localized("Stop", table: "Sidekick") : String.localized("Voice", table: "Sidekick"),
            action: { handleMicrophonePress() },
            background: audioRecorder.isRecording ? Color.red500.opacity(0.15) : Color.clear,
            hoverBackground: audioRecorder.isRecording ? Color.red500.opacity(0.3) : Color.T_8,
            fontColor: audioRecorder.isRecording ? Color.red500 : Color.S_2
        )
        ...
        .alert(String.localized("OpenAI API Key Required", table: "Sidekick"), isPresented: $showingAPIKeyAlert) {
            Button(String.localized("Open Settings", table: "Sidekick")) {
                NSWorkspace.shared.open(URL(string: "onit-sidekick://settings/general")!)
            }
            Button(String.localized("Cancel", table: "Sidekick"), role: .cancel) {}
        } message: {
            Text(String.localized("Please set your OpenAI API key in Settings to use dictation.", table: "Sidekick"))
        }
        .alert(String.localized("Dictation Error", table: "Sidekick"), isPresented: $showingErrorAlert) {
            Button(String.localized("OK", table: "Sidekick"), role: .cancel) {}
        } message: {
            Text(errorMessage)
        }
        .alert(String.localized("Microphone Access Required", table: "Sidekick"), isPresented: $showingMicrophonePermissionAlert) {
            Button(String.localized("Open System Settings", table: "Sidekick")) {
                NSWorkspace.shared.open(URL(string: "x-apple.systempreferences:com.apple.preference.security?Privacy_Microphone")!)
            }
            Button(String.localized("Cancel", table: "Sidekick"), role: .cancel) {}
        } message: {
            Text(String.localized("Onit needs access to your microphone for voice input. Please enable it in System Settings.", table: "Sidekick"))
        }
        .alert(String.localized("Recording Error", table: "Sidekick"), isPresented: Binding<Bool>(
            ...
            Text(audioRecorder.recordingError?.localizedDescription ?? String.localized("An unknown error occurred", table: "Sidekick"))
        }
    }
}
...
extension MicrophoneButton {
    private var microphoneButtonHelpText: String {
        if !audioRecorder.permissionGranted {
            return String.localized("Click to enable microphone access", table: "Sidekick")
        } else if audioRecorder.isRecording {
            return String.localized("Stop recording", table: "Sidekick")
        } else {
            return String.localized("Start voice recording", table: "Sidekick")
        }
    }
}
