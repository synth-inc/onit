...
class ContextWindowController: NSObject, NSWindowDelegate {
    ...
    init?(windowState: OnitPanelState, context: Context) {
        ...
        case .web(let websiteUrl, let websiteTitle, _):
            ...
            self.webFileContents = String.localized("Contents for %@ are still loading.", table: "Sidekick", websiteUrlDomain)

            let websiteContentsUnavailableText = String.localized("Website contents not available.", table: "Sidekick")
