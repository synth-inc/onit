...
struct TetheredButton: View {
    ...
    private var fitActiveWindowPrompt: String {
        return String.localized("Close Onit", table: "Sidekick")
    }
    ...
    var body: some View {
        VStack {
            ...
            .background(
                self.isHovering ? Color.baseBG.opacity(0.5) : Color.baseBG.opacity(0.7)
            )
            .background(Backgrounds.BrushedGlass())
            .cornerRadius(Self.width / 2)
            .addAnimation(dependency: self.isHovering)
            .onHover { hovering in
                isHovering = hovering
