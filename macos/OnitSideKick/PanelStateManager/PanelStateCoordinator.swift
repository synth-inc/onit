...
class PanelStateCoordinator {
    ...
    func configure(frontmostPidAtLaunch: pid_t?) {
        ...
        .sink { [weak self] enableSidebar, _ in
            guard let self = self else { return }

            if HintManager.shared.shouldShowHint {
                self.currentManager.start()
            } else {
                self.currentManager.stop()
            }

            /// Sidebar.
            let sidebarEnabled = enableSidebar.newValue
            if sidebarEnabled {
    ...
    func launchPanel(for state: OnitPanelState? = nil, createNewChat: Bool = true) {
        guard Defaults[.enableSidebar] else { return }
        guard FeatureDisableManager.shared.isEnabled(.sidekick) else { return }
