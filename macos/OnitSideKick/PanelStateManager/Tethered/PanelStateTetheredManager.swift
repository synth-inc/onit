...
class PanelStateTetheredManager: PanelStateBaseManager, ObservableObject {
    ...
    override func start() {
        ...
        /// Activate the current frontmost window to show the hint on startup.
        /// Without this, switching from Pinned to Tethered mode wouldn't show the hint because no new window activation notification is sent.
        if let pid = NSWorkspace.shared.frontmostApplication?.processIdentifier,
           let tracked = AccessibilityNotificationsManager.shared.windowsManager.trackWindowForElement(
               pid.getAXUIElement(),
               pid: pid
           ) {
            let panelState = getState(for: tracked)
    ...
    @objc func appDidBecomeActive(_ notification: Notification) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            /// Check if any panel is the key window.
            ...
            /// In this case, no panel is the key window. Meaning, we should hide the hint.
            /// This allows us to prevent the hint from appearing above Onit windows while in tethered mode.
            /// For example, dragging the Settings window above an app window that has the hint would prevent the hint from oddly floating above it.
        ...
        if !state.hidden {
            if state.panelOpened {
                ...
                if Defaults[.enableSidebar] {
                    KeyboardShortcutsManager.enablePanelShortcuts(modelContainer: SwiftDataContainer.appContainer)
                }
                ...
                // Panel closed - only manage shortcuts if sidebar is enabled
                if Defaults[.enableSidebar] {
                    KeyboardShortcutsManager.disablePanelShortcuts(modelContainer: SwiftDataContainer.appContainer)
                }
