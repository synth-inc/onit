    ...
    var body: some View {
        VStack {
            ...
            VStack(alignment: .center, spacing: 8) {
                Text(String.localized("Welcome to Onit", table: "Onboarding"))
                    .styleText(size: 19, weight: .medium)
                Text(String.localized("Click to start", table: "Onboarding"))
                    .styleText(size: 12, color: Color.S_2)
            }
            .padding([.top, .bottom, .leading], 20)
            .padding(.trailing, 40)
            .background(TutorialShape().fill(Color.S_7))
            .overlay(TutorialShape().stroke(Color.S_5, lineWidth: 1))
