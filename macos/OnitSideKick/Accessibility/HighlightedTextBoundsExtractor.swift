...
class HighlightedTextBoundsExtractor {
    ...
    private func calculateBounds(for element: AXUIElement, selectedText: String) async -> HighlightedTextBoundsResult? {
        ...
        // 4: approximation (fallback)
            ...
            // Fallback to mouse location - more accurate than element frame since
            // the user likely just selected text with the mouse or is typing nearby
            let mouseLocation = NSEvent.mouseLocation
            // Convert from bottom-left origin to top-left origin for screen coordinates
            if let screen = NSScreen.main {
                let screenHeight = screen.frame.height
                let mouseY = screenHeight - mouseLocation.y
                // Create a small rectangle around the mouse position
    ...
    private func calculateFrameUsingOCR(element: AXUIElement, selectedText: String) async -> CGRect? {
        ...
        guard let screenshot = await captureElementScreenshot(frame: elementFrame) else {
    ...
    private func captureElementScreenshot(frame: CGRect) async -> CGImage? {
        // Convert from AX/Quartz coordinates (top-left origin) to AppKit/macOS coordinates
        // (bottom-left origin) that ScreenCaptureKit expects. This also avoids the
        // "Onit is trying to record your screen" popup triggered by CGDisplayCreateImage.
        let appKitFrame = frame.toMacOSCoordinates()
        return await ScreenRecordingManager.captureScreenRegion(at: appKitFrame)
    }
