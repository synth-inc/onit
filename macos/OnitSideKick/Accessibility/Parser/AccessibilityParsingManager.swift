...
@MainActor
final class AccessibilityParsingManager: AccessibilityNotificationsDelegate {
    ...
    private init() {
        // Register as delegate for accessibility notifications
    ...
    func requestParsing(
        ...
        if let cachedResult = validateAndCleanExpiredResult(for: elementHash) {
            log.warning("AccessibilityParsingManager: Using cached results for element hash \(elementHash)")
    ...
    func requestEnhancedParsing(
        ...
        if let cachedResult = validateAndCleanExpiredResult(for: elementHash),
           cachedResult.hasBoundingBoxes {
            log.warning("AccessibilityParsingManager: Using cached enhanced results for element hash \(elementHash)")
    ...
    func clearCache() {
        ...
        log.warning("AccessibilityParsingManager: Cleared all cached results")
    }
    ...
    func clearCache(for windowElement: AXUIElement) {
        ...
        log.warning("AccessibilityParsingManager: Cleared cached results for element hash \(elementHash)")
    }
    ...
    func invalidateCache(for windowElement: AXUIElement, reason: String) {
        ...
        if cachedResults.removeValue(forKey: elementHash) != nil {
            log.warning("AccessibilityParsingManager: Invalidated cached results for element hash \(elementHash), reason: \(reason)")
        }
    }
    ...
    func invalidateCache(for pid: pid_t, reason: String) {
        ...
        if !keysToRemove.isEmpty {
            log.warning("AccessibilityParsingManager: Invalidated \(keysToRemove.count) cached results for PID \(pid), reason: \(reason)")
        }
    }
    ...
    /// - Parameters:
    ///   - windowElement: The AXUIElement to parse
    ///   - ignoreCache: If true, bypasses cache and always performs fresh parsing
    func parseElement(_ windowElement: AXUIElement, ignoreCache: Bool = false) async -> [String: String] {
        let shouldBlock = await MainActor.run {
            PrivacyManager.shared.shouldBlockDataCollection()
        }
        ...
        if !ignoreCache, let cachedResult = validateAndCleanExpiredResult(for: elementHash) {
            log.warning("AccessibilityParsingManager: Direct parse using cached results for element hash \(elementHash)")
        ...
        do {
            ...
            log.warning("AccessibilityParsingManager: Direct parse cached results for element hash \(elementHash)")
            
            return results
        } catch {
            log.warning("AccessibilityParsingManager: Error in direct parse: \(error)")
            return [:]
        }
    }
    ...
    func parseElementWithBoundingBoxes(_ windowElement: AXUIElement) async -> ([String: String], [TextBoundingBox]?) {
        ...
        if let cachedResult = validateAndCleanExpiredResult(for: elementHash),
           cachedResult.hasBoundingBoxes {
            log.warning("AccessibilityParsingManager: Direct enhanced parse using cached results for element hash \(elementHash)")
        ...
        do {
            ...
            cachedResults[elementHash] = cachedResult
            log.warning("AccessibilityParsingManager: Direct enhanced parse cached results for element hash \(elementHash)")
            
            return result
        } catch {
            log.warning("AccessibilityParsingManager: Error in direct enhanced parse: \(error)")
            return ([:], nil)
        }
    }
    ...
    private func startNewParseOperation(elementHash: UInt, windowElement: AXUIElement, initialRequest: ParseRequest) {
        ...
        log.warning("AccessibilityParsingManager: Starting new parse operation for element hash \(elementHash), needsBoundingBoxes: \(needsBoundingBoxes)")
    ...
    private func completeOperation(elementHash: UInt, result: Result<([String: String], [TextBoundingBox]?), Error>, hasBoundingBoxes: Bool, pid: pid_t?) {
        ...
        if case .success(let (results, boundingBoxes)) = result {
            ...
            cachedResults[elementHash] = cachedResult
            log.warning("AccessibilityParsingManager: Cached results for element hash \(elementHash)")
        }
    ...
    func splitTextAroundElement(_ targetElement: AXUIElement) async -> (precedingText: String, followingText: String) {
        let elementHash = CFHash(targetElement)
        
        if let pid = targetElement.pid(), let mainWindow = pid.firstMainWindow {
            
            // Get cached or fresh parsing results
            let fullText: String
            if let cachedResult = validateAndCleanExpiredResult(for: elementHash) {
                log.warning("AccessibilityParsingManager: Using cached results for text splitting")
                fullText = cachedResult.results["screen"] ?? ""
            } else {
                log.warning("AccessibilityParsingManager: Parsing fresh results for text splitting")
                do {
                    let (results, _) = try await AccessibilityParser.shared.getAllTextInElement(windowElement: mainWindow)
                    let cachedResult = CachedResult(results: results, boundingBoxes: nil, hasBoundingBoxes: false, pid: pid)
                    cachedResults[elementHash] = cachedResult
                    fullText = results["screen"] ?? ""
                } catch {
                    log.warning("AccessibilityParsingManager: Error parsing text for splitting: \(error)")
                    return ("", "")
                }
            }
            
            // Find the target element's value or closest element with value
            let (elementValue, elementPosition) = await findElementValueForSplitting(targetElement: targetElement, windowElement: mainWindow)
            
            guard !elementValue.isEmpty else {
                log.warning("AccessibilityParsingManager: No element value found for splitting")
                return ("", fullText)
            }
    ...
    private func findElementValueForSplitting(targetElement: AXUIElement, windowElement: AXUIElement) async -> (value: String, position: ElementPosition) {
        ...
        if let targetValue = targetElement.value(), !targetValue.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            log.warning("AccessibilityParsingManager: Using target element's own value for splitting")
    ...
    private func checkSiblingsForValue(of element: AXUIElement, pathFromTarget: [AXUIElement]) async -> (value: String, position: ElementPosition)? {
        ...
        // Find the index of our element among its siblings using CFHash equality
        guard let elementIndex = siblings.firstIndex(where: { CFHash($0) == CFHash(element) }) else {
            return nil
        }
        ...
        for i in stride(from: elementIndex - 1, through: 0, by: -1) {
            if let value = await searchElementAndChildrenForValue(siblings[i]) {
                log.warning("AccessibilityParsingManager: Found preceding sibling with value")
        ...
        for i in (elementIndex + 1)..<siblings.count {
            if let value = await searchElementAndChildrenForValue(siblings[i]) {
                log.warning("AccessibilityParsingManager: Found following sibling with value")
    ...
    /// Search an element and its descendants for a value (depth-first), up to a max depth of 3
    private func searchElementAndChildrenForValue(_ element: AXUIElement, depth: Int = 0) async -> String? {
        // Limit recursion to a max depth of 3
        if depth > 3 {
            return nil
        }
        ...
        for child in children {
            if let value = await searchElementAndChildrenForValue(child, depth: depth + 1) {
                return value
            }
        }
...
//            log.warning("AccessibilityParsingManager: Element value '\(elementValue)' not found in full text")
            return ("", fullText)
        }
    ...
    private func validateAndCleanExpiredResult(for elementHash: UInt) -> CachedResult? {
        ...
        if cachedResult.isValid {
            ...
            cachedResults.removeValue(forKey: elementHash)
            log.warning("AccessibilityParsingManager: Removed expired cached result for element hash \(elementHash)")
            return nil
        }
    }

    private func startPeriodicCleanup() {
        // Clean up every 60 seconds (adjust interval as needed)
    ...
    private func cleanupExpiredCacheEntries() {
        let expiredKeys = cachedResults.compactMap { (key, value) -> UInt? in
            return value.isValid ? nil : key
        }
        ...
        if !expiredKeys.isEmpty {
            log.warning("AccessibilityParsingManager: Cleaned up \(expiredKeys.count) expired cache entries")
        }
    }
